  ...
  Future<void> _assertYamlBasilValid(YamlMap? yamlMap) async {
    ...
    for (String buildType in yamlMapBasil.keys) {
      ...
      if (yamlBuildType['cmds'] is! YamlList) {
        throw ArgumentError(
            'Field in "$buildType" is not of type YamlList', 'cmds');
      }

      if (yamlBuildType['enabled'] != null &&
          yamlBuildType['enabled'] is! bool) {
        throw ArgumentError(
            'Field in "$buildType" is not of type bool', 'enabled');
      }

      if (yamlBuildType['parallel'] != null &&
          yamlBuildType['parallel'] is! bool) {
        throw ArgumentError(
            'Field in "$buildType" is not of type bool', 'parallel');
      }

      if (yamlBuildType['platforms'] != null &&
          yamlBuildType['platforms'] is! YamlList) {
        throw ArgumentError(
            'Field in "$buildType" is not of type YamlList', 'platforms');
      }
    }
  }

  /// Return the contents of [_yamlFile] as a string.
  Future<String> _getYamlString() async {
    await _assertYamlFileExists();
    return await _yamlFile.readAsString();
  }
